...
export const PlayHub: React.FC = () => {
  ...
  const handleMoveMade = (move: { from: string; to: string; san: string; promotion?: string }) => {
    ...
    // Add increment to white
    setWhiteTime((prev) => prev + increment);
    setCurrentTurn('b');

    // If bot mode, let bot play
    if (mode === 'bot') {
      triggerBotMove();
    }
  };

  const handleGameOver = (result: { winner: 'w' | 'b' | 'draw'; reason: string; pgn: string }) => {
    setIsClockRunning(false);
    ...
    const resString = isWin ? '1-0' : isLoss ? '0-1' : '1/2-1/2';
    ...
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
        ...
        <div className="md:col-span-3 flex items-center gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-2xl bg-[#101b33] hover:bg-[#162444] border border-blue-500/30 text-xs font-bold text-slate-200 transition-colors"
          >
          ...
          <button
            ...
            title="Analyze match with Stockfish 18 & Gemini"
          >
      ...
      {isCustomTime && (
        <div className="p-4 rounded-2xl bg-[#0c1427] border border-amber-500/40 shadow-xl flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            onClick={handleStartNewGame}
            className="px-4 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold text-xs transition-all"
          >
            Apply & Start
          </button>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-bold text-white flex items-center gap-1.5 font-cinzel">
                  {mode === 'bot' ? 'Computer' : 'Player 2 (Black)'}
                  {mode === 'bot' && (
                ...
                <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono">
                  <span>⚡ {mode === 'bot' ? botElo : 1500} Elo</span>
                </div>
              </div>
            </div>
          ...
          {gameStatus && (
            <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-950/90 to-red-950/90 border-2 border-amber-500/80 text-center space-y-2 shadow-2xl">
              ...
              <div className="flex items-center justify-center gap-2 pt-1">
                <button
                  onClick={handleStartNewGame}
                  className="py-1.5 px-3 rounded-xl bg-gradient-to-r from-blue-600 to-red-600 text-white font-bold text-xs shadow-md"
                >
                  Play Rematch
                </button>
                <button
                ...
                >
                  <Sparkles size={13} />
                  <span>Review with Stockfish & Gemini</span>
                </button>
              </div>
            </div>
          )}
